...
export default function BizSimHub() {
    ...
    <nav className={`navbar ${transparent ? 'navbar-transparent' : ''}`}>
      <div className="nav-container">
        ...
        <div className="nav-links">
          ...
          {!currentUser ? (
            <>
              {!transparent && <button className="nav-link" onClick={() => setCurrentPage('pricing')}>{t('nav.pricing', lang)}</button>}
  ...
  const renderLanding = () => (
    <div className="landing-page">
      ...
      <section className="testimonials-section" style={{ padding: '5rem 2rem', background: 'var(--bg-card)' }}>
        <div className="section-container" style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <h2 style={{ textAlign: 'center', marginBottom: '3rem', fontSize: '2rem' }}>{t('landing.testimonialTitle', lang)}</h2>
          <div className="testimonials-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
            <div className="testimonial-card" style={{ background: 'var(--bg-secondary)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', marginBottom: '1rem' }}>
                {'⭐'.repeat(5)}
              </div>
              <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem', fontStyle: 'italic' }}>
                {lang === 'en' 
                  ? '"The simulations helped me understand project management in a way textbooks never could. I made decisions, saw consequences, and learned faster."'
                  : '"Les simulations m\'ont aidé à comprendre la gestion de projet d\'une manière que les manuels ne pouvaient jamais faire. J\'ai pris des décisions, vu les conséquences et appris plus vite."'
                }
              </p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>MR</div>
                <div>
                  <div style={{ fontWeight: '600' }}>Marc R.</div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{lang === 'en' ? 'Project Manager, Tech Industry' : 'Chef de projet, Industrie Tech'}</div>
                </div>
              </div>
            </div>
            <div className="testimonial-card" style={{ background: 'var(--bg-secondary)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border)' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem', fontStyle: 'italic' }}>
                {lang === 'en'
                  ? '"I use BizSimHub to train my team. The realistic scenarios prepare them for challenges before they happen in real projects."'
                  : '"J\'utilise BizSimHub pour former mon équipe. Les scénarios réalistes les préparent aux défis avant qu\'ils ne surviennent dans de vrais projets."'
                }
              </p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, #10b981, #059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>SL</div>
                <div>
                  <div style={{ fontWeight: '600' }}>Sophie L.</div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{lang === 'en' ? 'Director of PMO, Montreal' : 'Directrice PMO, Montréal'}</div>
                </div>
              </div>
            </div>
            <div className="testimonial-card" style={{ background: 'var(--bg-secondary)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border)' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem', fontStyle: 'italic' }}>
                {lang === 'en'
                  ? '"Finally, a way to practice PM skills without the stress of failing on a real project. The feedback is immediate and actionable."'
                  : '"Enfin, un moyen de pratiquer les compétences PM sans le stress d\'échouer sur un vrai projet. Les retours sont immédiats et exploitables."'
                }
              </p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, #f59e0b, #d97706)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>JT</div>
                <div>
                  <div style={{ fontWeight: '600' }}>Jean T.</div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{lang === 'en' ? 'PMP Candidate, Quebec City' : 'Candidat PMP, Ville de Québec'}</div>
                </div>
              </div>
            </div>
          </div>
